-- Enforce personal watchlist limit at the DB level (3 for free/visitor,
-- unlimited for pro/elite/admin) — the frontend already checks this before
-- calling supabase.from('watchlist').insert(), but that check runs client-side
-- and is bypassable via devtools, since RLS on watchlist only checks
-- user_id = auth.uid(), never plan.
--
...
-- read the same current_count and both pass, exceeding the limit by one row
-- in that race. Not worth the added complexity for a personal-watchlist soft
-- cap.
--
...
BEGIN
  ...
  ELSE
    max_allowed := 3; -- free/visitor plan
  END IF;
...
WHERE trigger_name = 'enforce_watchlist_limit';
